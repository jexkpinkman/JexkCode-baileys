...
// JexkCode@Update - Extract product message into a standalone function so it can also be reused as the header for interactive messages
const prepareProductMessage = async (message, options) => {
    ...
    // JexkCode@Update - Add product message default value
 ...
 */
// JexkCode@Update - Enhanced prepareStickerPackMessage
const prepareStickerPackMessage = async (message, options) => {
    const { cover, stickers = [], name = '📦 Sticker Pack', publisher = 'GitHub: jexkpinkman', description = '🏷️ jexkpinkman/JexkCode-baileys' } = message;
    ...
    const logger = options.logger;
    // JexkCode@Update - Add caching for sticker pack
...
// JexkCode@Update - Add native flow button helper for interactive message
const prepareNativeFlowButtons = (message) => {
    ...
    // JexkCode@Update - Add offer and options inside interactive message
    if (hasOptionalProperty(message, 'offerText') && !!message.offerText) {
        Object.assign(messageParamsJson, {
            limited_time_offer: {
                ...
                url: message.offerUrl || DONATE_URL, // JexkCode@Note - Apologies if this feels cheeky, just a fallback
    ...
    return {
        buttons: correctedField.map(button => {
            ...
            // JexkCode@Update - Add "single_select" shortcut ＼⁠(⁠°⁠o⁠°⁠)⁠／
...
// JexkCode@Update - Validate album message media to avoid bug 👀
...
// JexkCode@Update - Validate carousel cards header to avoid bug 👀
...
export const generateWAMessageContent = async (message, options) => {
    var _a, _b;
    let m = {};
    // JexkCode@Update - Add "raw" boolean to send raw messages directly via generateWAMessage()
    if (hasNonNullishProperty(message, 'raw')) {
        delete message.raw;
        return message;
    }
    // JexkCode@Update - Add support for code block and table with richResponseMessage
        ...
        if (message.poll.toAnnouncementGroup) {
            ...
            // JexkCode@Update - Add quiz message support
            if (message.poll.pollType === 1) {
                ...
                m.pollCreationMessageV5 = {
                    // JexkCode@Note - quiz for newsletter only
                    ...pollCreationMessage,
                    correctAnswer: {
    ...
    // JexkCode@Update - Add poll result snapshot message
    ...
    // JexkCode@Update - Add poll update message
    ...
    // JexkCode@Update - Add payment invite message
    ...
    // JexkCode@Update - Add order message
    ...
    // JexkCode@Update - Add interactive messages (buttonsMessage, listMessage, interactiveMessage, templateMessage, and carouselMessage)
    if (hasNonNullishProperty(message, 'buttons')) {
        const buttonsMessage = {
            buttons: message.buttons.map(button => {
                // JexkCode@Update - Add "single_select" shortcut!
                const buttonText = button.text || button.buttonText;
    ...
    // JexkCode@Note - This message type is shown on WhatsApp Web/Desktop and iOS (I guess ｡⁠◕⁠‿⁠◕⁠｡). On Android, it only appears in newsletter (so far ಥ⁠‿⁠ಥ)
        ...
        hydratedTemplate.templateId = message.id || 'template-' + Date.now(); // JexkCode@Note - Minimal templateId to satisfy WhatsApp (⁠ ⁠ꈍ⁠ᴗ⁠ꈍ⁠)
        m = {
            templateMessage: {
    ...
    // JexkCode@Update - Add request payment message
    ...
    // JexkCode@Update - Add invoice message
    ...
    // JexkCode@Update - Add direct externalAdReply access (no need to create contextInfo first)
    if (hasOptionalProperty(message, 'externalAdReply') && !!message.externalAdReply) {
        ...
        if (!content.url || typeof content.url !== 'string') {
            content.url = DONATE_URL; // JexkCode@Note - Apologies if this feels cheeky, just a fallback
        }
    ...
    // JexkCode@Update - Add "groupStatus" boolean to set contextInfo.isGroupStatus and wrap message into groupStatusMessageV2
    ...
    // JexkCode@Update - Add "spoiler" boolean to set contextInfo.isSpoiler and wrap message into spoilerMessage
    ...
    // JexkCode@Update - Add "interactiveAsTemplate" boolean to wrap interactiveMessage into templateMessage
    else if (hasOptionalProperty(message, 'interactiveAsTemplate') && !!message.interactiveAsTemplate) {
        if (!m.interactiveMessage) {
            throw new Boom('Invalid message type for template', { statusCode: 400 }); // JexkCode@Note - To avoid bug 👀
        }
        m = {
            templateMessage: {
                interactiveMessageTemplate: m.interactiveMessage,
                templateId: message.id || 'template-' + Date.now() // JexkCode@Note - Minimal templateId to satisfy WhatsApp (⁠ ⁠ꈍ⁠ᴗ⁠ꈍ⁠)
            }
        };
        delete message.interactiveAsTemplate;
    }
    // JexkCode@Update - Add "ephemeral" boolean to wrap message into ephemeralMessage like "viewOnce"
    if (hasOptionalProperty(message, 'ephemeral') && !!message.ephemeral) {
        m = { ephemeralMessage: { message: m } };
        delete message.ephemeral;
    }
    // JexkCode@Update - Add wrap message into lottieStickerMessage by isLottie boolean
    ...
    // JexkCode@Update - Add "viewOnceV2" boolean to wrap message into viewOnceMessageV2 like "viewOnce"
    else if (hasOptionalProperty(message, 'viewOnceV2') && !!message.viewOnceV2) {
        m = { viewOnceMessageV2: { message: m } };
        delete message.viewOnceV2;
    }
    // JexkCode@Update - Add "viewOnceV2Extension" boolean to wrap message into viewOnceMessageV2Extension like "viewOnce"
...
export const generateWAMessageFromContent = (jid, message, options) => {
    ...
    // JexkCode@Update - Add deviceListMetadata inside messageContextInfo for private chat
...
export const normalizeMessageContent = (content) => {
    ...
    return content;
    // JexkCode@Update - Add all futureProofMessage into getFutureProofMessage()
