...
// JexkCode@Update - Inject buffer into unifiedResponse.data to support proper rendering of rich messages (ex: tables and code blocks)
...
export const prepareRichResponseMessage = (content) => {
    ...
    const richResponseMessage = proto.AIRichResponseMessage.create({
        ...
        unifiedResponse: {
            data: Buffer.from(JSON.stringify(unified)) // JexkCode@Note - Expects "ArrayBufferLike"
        },
        contextInfo: {
            isForwarded: true,
            forwardingScore: 1,
    ...
    // JexkCode@Note - Add disclaimer text on richResponseMessage
    if (disclaimerText) {
        botMetadata.messageDisclaimerText = disclaimerText;
    }
    // JexkCode@Note - Add responseId from unified directly to botMetadata
    botMetadata.botResponseId = uuid;
    return message;
};
// JexkCode@Note - signature and certificateChain for proofs[] field
...
export const wrapToBotForwardedMessage = (richResponseMessage) => ({
    messageContextInfo: {
        botMetadata: {
            // JexkCode@Note - TODO: Fill verificationMetadata field
